...
//Receive Message
int readData(char *buf, int sz_buf, int timeout) {
  int len=0;
  int flag=0;
  int datalen=0;
  char work[4];
  ...
  while( (time+timeout) > millis()) {
    while(espSerial->available())  {
      ...
      if (flag == 0) {
        //Check if it is the letter +
        if (current_byte != 0x2B) continue;
        flag++;
        
      } else if (flag == 1) {
        //Check if it is the letter I
        if (current_byte == 0x49) {
          flag++;
        } else {
          flag = 0;
        }
        ...
        //Check if it is the letter D
        if (current_byte == 0x44) {
          flag++;
        } else {
          flag = 0;
        }
